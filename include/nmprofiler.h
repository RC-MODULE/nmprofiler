...
int main()
{
 ...
 \par 6. Непосредственный доступ к результатам профилирования 
  ...
  \code 
  S_ProfilerData* profile_item=nmprofiler_head(); 
  \endcode 
  ...
  секцию .text_profiler может располагать в недоступной извне  внутренней памяти. В этих целях рекомендуется использоваться функцию копирования списка структур S_ProfilerData во внешнюю разделяемую память с помощью функции nmprofiler_list2table(). 
...
Файл profile.xsl при этом должен находится рядом с файлом profile.xml.
 ...
 \par Примечание 2 :
Названия функций хранятся в массиве типа int поля S_ProfilerData::funcname. Для корректного отображения строки на host-машине необходимо пользоваться функциями преобразования к типу char: nmprofiler_char8bit() или функцией членом класса S_ProfilerData::funcname2char().
...
*/
 ...
 //! \{
	...
	void nmprofiler_sprintf(char* dststr, S_ProfilerData* prof, char* format, char* full_func_name=0);
...
//	void nmprofiler_last();
	...
	*/
	...
	//! \param head указатель на первую структуру в списке.
	void  nmprofiler_print2tbl(S_ProfilerData* head=0);
	...
	//! \param head указатель на первую структуру в списке.
	void  nmprofiler_print2xml(S_ProfilerData* head=0);

	void nmprofiler_enable();
	void nmprofiler_disable();
	int  nmprofiler_count();
	void nmprofiler_copy(S_ProfilerData* dst);
#endif	
